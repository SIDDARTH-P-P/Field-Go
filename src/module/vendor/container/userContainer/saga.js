...
//           const allowedRoles = ['vendorAcnt', 'vendorMngr', 'vendorOptr'];
//           const filteredData = response.filter(item => allowedRoles.includes(item.role));
...
function* addUser(action) {
  try {
    const { orgId, ...userDetails } = action.payload; 
    console.log("========post users12 ==========",orgId );
    console.log("========post users ==========",action.payload );
    ...
    const params = {
      ...
      authorization: 'token',
      body: JSON.stringify({ orgId, ...userDetails }), 
    };

    const res = yield call(auth.basicApi, params);

    if (res) {
      yield put(actionType.getUser()); // Fetch updated user list
      yield call(() => toast.success('User added successfully', { autoClose: 3000 }));
      yield put(actionType.totalCount({ where: {} })); // Update total count if needed
    }
  } catch (error) {
    console.error('Error adding user:', error);
    yield call(() => toast.error('Failed to add user', { autoClose: 3000 }));
    yield put(actionType.addUserFail()); // Dispatch failure action
  }
}
